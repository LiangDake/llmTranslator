...
def singlechunk_translation(
        target_lang: str, source_text: str
) -> str:
    system_message = f"您是一位语言专家，擅长将所有语言翻译成 {target_lang}."

    translation_prompt = f"""这是任意语言到{target_lang}的翻译，请提供此文本的{target_lang}翻译。\
    除翻译外，请勿提供任何解释或文字。
    {source_text}
    {target_lang}:
...
def multichunk_translation(
        target_lang, source_text_chunks
) -> List[str]:
    system_message = f"您是一位语言专家，擅长将所有语言翻译成{target_lang}."

    translation_prompt = """您的任务是提供文本部分转到{target_lang}的专业翻译。
