...
from localbin.translator import translate_base
...
class QueryFileBase(QueryKnowledgeBase):
    ...
    def translate_file_without_eml(self, file_path: str, translated_folder_path, source_lang='osd', target_lang='中文'):
        # 实现翻译逻辑
        # 获取文件中的文字
        doc = file_processing.import_file(file_path, lang=source_lang)
        # 确保格式正确
        if doc is not False:
            source_text = doc[0].page_content
            translation = translate_base.translate(
                target_lang, source_text=source_text
            )
            # 获取文件类型
            file_type = file_processing.get_file_type(file_path)
            if file_type in ('.docx', 'doc'):
                # 如果是 docx 文件，调用 save_translated_docx 保存
    ...
    def translate_email(self, file_path: str, translated_folder_path, target_lang='中文'):
        ...
        # 重新创建新的EML
        translated_eml_path = file_processing.save_translated_email(
            file_path,
            email_headers,
            translation,
            translated_folder_path,
...
class GraphBase(QueryFileBase):
    ...
    def build_graph(self, folder_path: str): # 读取文件夹中所有文件
